...
void ItemDataBase::LoadItemData() {
    ...
    for (const auto& item : data) {
        ...
        if (!enumDisplay.GetItemCategoryToEnum(item["category"],
                itemData.category)) {
            return;
        }
...
void ItemDataBase::LoadConsumableData() {
    ...
    for (const auto& item : data) {
        ...
        for (const auto& effect : item["effects"]) {
            ...
            if (!enumDisplay.GetConsumableTypeToEnum(
        ...
        consumableDataMap.emplace(consumableData.GetId(),
                                  consumableData);
    }
}
...
void ItemDataBase::LoadUpgradeData() {
    ...
    for (const auto& item : data) {
        ...
        if (!enumDisplay.GetUpgradeTypeToEnum(
                item["upgradeType"],
                upgradeItemData.upgradeType)) {
            return;
        }
        ...
        upgradeDataMap.emplace(upgradeItemData.GetId(),
                               upgradeItemData);
    }
}
...
void ItemDataBase::LoadMaterialData() {
    ...
    for (const auto& item : data) {
        ...
        if (!enumDisplay.GetMaterialTypeToEnum(
                item["materialType"],
                materialItemData.materialType)) {
            return;
        }

        materialItemData.SetId(item["id"]);

        materialDataMap.emplace(materialItemData.GetId(),
                                materialItemData);
    }
}
...
void ItemDataBase::PrintAllItemData() const {
    ...
    for (std::unordered_map<std::string,
                            FItemData>::const_iterator iter =
                                itemDataMap.begin();
                                iter != itemDataMap.end();
                                iter++) {
        std::cout <<
            std::format("ID: {}",
                         iter->first) << std::endl;
        std::cout <<
            std::format("id: {}",
                         iter->second.GetId()) << std::endl;
        std::cout <<
            std::format("category: {}",
                         enumDisplay.
                         GetItemCategoryToString(
                         iter->second.GetCategory())) << std::endl;
        std::cout <<
            std::format("description: {}",
                         iter->second.GetDescription()) << std::endl;
        std::cout <<
            std::format("name: {}",
                         iter->second.GetName()) << std::endl;
        std::cout <<
            std::format("price: {}",
                         iter->second.GetPrice()) << std::endl;
        std::cout <<
            std::format("maxStackCount: {}",
                         iter->second.GetMaxStackCount()) << std::endl;
    }
}
...
void ItemDataBase::PrintAllConsumableData() const {
    ...
    for (std::unordered_map<std::string,
                            FConsumableItemData>::const_iterator iter =
            consumableDataMap.begin();
            iter != consumableDataMap.end();
            iter++) {
        ...
        for (const FConsumableEffect& effect :
                iter->second.GetConsumableEffects()) {
            std::cout <<
                std::format("consumableType: {}, value: {}",
                             enumDisplay.
                             GetConsumableTypeToString(
                             effect.GetConsumableType()),
                             effect.GetValue()) << std::endl;
        }
    }
}
...
void ItemDataBase::PrintAllUpgradeData() const {
    ...
    for (std::unordered_map<std::string,
                            FUpgradeItemData>::const_iterator iter =
            upgradeDataMap.begin();
            iter != upgradeDataMap.end();
            iter++) {
        std::cout <<
            std::format("ID: {}", iter->first) << std::endl;
        std::cout <<
            std::format("id: {}", iter->second.GetId()) << std::endl;
        std::cout <<
            std::format("upgradeType: {}",
                         enumDisplay.
                             GetUpgradeTypeToString(
                                 iter->second.GetUpgradeType())) << std::endl;
        std::cout <<
            std::format("value: {}",
                         iter->second.GetValue()) << std::endl;
    }
}
...
void ItemDataBase::PrintAllMaterialData() const {
    ...
    for (std::unordered_map<std::string,
                            FMaterialItemData>::const_iterator iter =
            materialDataMap.begin();
            iter != materialDataMap.end();
            iter++) {
        std::cout << std::format("ID: {}", iter->first) << std::endl;
        std::cout << std::format("id: {}", iter->second.GetId()) << std::endl;
        std::cout <<
            std::format("materialType: {}",
                         enumDisplay.
                         GetMaterialTypeToString(
                         iter->second.GetMaterialType())) << std::endl;
    }
}
...
const std::vector<std::string> ItemDataBase::GetAllItemIds() const {
    ...
    for (std::unordered_map<std::string, FItemData>::const_iterator iter =
            itemDataMap.begin();
            iter != itemDataMap.end();
            iter++) {
...
bool ItemDataBase::GetDescription(std::string id,
...
bool ItemDataBase::GetConsumableData(
                    std::string id,
...
bool ItemDataBase::GetConsumableEffects(
                    std::string id,
                    std::vector<FConsumableEffect>& consumbaleEffects) const {
    if (!consumableDataMap.contains(id)) {
        return false;
    }
...
bool ItemDataBase::GetConsumableTypes(
                    std::string id,
                    std::vector<EConsumableType>& consumableTypes) const {
    if (!consumableDataMap.contains(id)) {
        return false;
    }
...
bool ItemDataBase::GetUpgradeData(std::string id,
                                  FUpgradeItemData& upgradeItemData) const {
...
bool ItemDataBase::GetUpgradeType(std::string id,
...
bool ItemDataBase::GetMaterialData(std::string id,
                                   FMaterialItemData& materialItemData) const {
...
bool ItemDataBase::GetMaterialType(std::string id,
