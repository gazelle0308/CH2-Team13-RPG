...
inline bool compareMaterial(
        const FPotionRecipeState* a,
        const FPotionRecipeState* b) {
            if (a->potionRecipe.GetPrimaryCount() ==
                b->potionRecipe.GetPrimaryCount()) {
                return a->potionRecipe.GetSecondaryCount() >
                       b->potionRecipe.GetSecondaryCount();
            }

    return a->potionRecipe.GetPrimaryCount() >
            b->potionRecipe.GetPrimaryCount();
}
...
void HandlePrimaryCount(
    ...
    void HandleSecondaryCount(
        int primaryIndex,
        int primaryCount,
        int secondaryIndex);

    void HandleCraftPotionResult(
        int primaryIndex,
        int primaryCount,
        int secondaryIndex,
        int secondaryCount);
